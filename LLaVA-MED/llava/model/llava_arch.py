...
class LlavaMetaForCausalLM(ABC):
    ...
    def prepare_inputs_labels_for_multimodal(
        ...
        if True:
            __input_ids = torch.concat([input_ids[:, :5], input_ids[:, 6:]], dim=1)
            __input_embeddings = self.get_model().embed_tokens(__input_ids)
            image_features = self.vdz(image_features, __input_embeddings)
        ...
        # 你一个注释都不写，而且下面的代码又混杂了多个处理逻辑，咋来给你提意见
        _labels = labels
