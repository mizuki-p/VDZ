...
from dataloader import RadDataset, SlakeDataset, OvqaDataset
from transformers import AutoTokenizer

output_file = 'recycle_bin/acc/ovqa_vdz_baseline.txt'
...
for response_item, label_item in zip(doc, dataset):
    ...
    if dataset.get_answer_type(label_item['qid']) == 'CLOSED':
        ...
        recall = sum([1 for rid in response_ids if rid in label_ids]) / (len(response_ids) if len(response_ids) > 0 else 1)
