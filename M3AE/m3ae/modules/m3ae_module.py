...
from m3ae.modules.language_encoders.bert_model import BertCrossLayerWithVDZ
...
#     ) -> torch.Tensor:
#         cls_embs = vision_embeddings[:, 0:1]
#         vision_embeddings = vision_embeddings[:, 1:]
#         batch_size, text_seq_len, hidden_dim = text_embeddings.shape
#         _, vision_seq_len, _ = vision_embeddings.shape
#         head_dim = hidden_dim // self.num_head
        
#         text_embeddings = text_embeddings.clone().detach_()
        
#         text_embeddings = text_embeddings. \
#             reshape(batch_size, text_seq_len, self.num_head, head_dim). \
#             transpose(1, 2). \
#             reshape(batch_size * self.num_head, text_seq_len, head_dim)
#         vision_embeddings = vision_embeddings. \
#             reshape(batch_size, vision_seq_len, self.num_head, head_dim). \
#             transpose(1, 2). \
#             reshape(batch_size * self.num_head, vision_seq_len, head_dim)
        
#         dist = torch.cdist(vision_embeddings, text_embeddings, p=2)
#         dist = dist.sum(dim=2)
        
#         near_to_far_indices = torch.argsort(dist, dim=1) 
#         sorted_vision_embeddings = torch.take_along_dim(vision_embeddings, dim=1, indices=near_to_far_indices.unsqueeze(-1))

#         # pooling 
#         if True:
#             _b, _l, _h = sorted_vision_embeddings.shape
#             sorted_vision_embeddings = sorted_vision_embeddings.reshape(_b, _l, 1, _h)
#             sorted_vision_embeddings = torch.concat([sorted_vision_embeddings, sorted_vision_embeddings], dim=2)
#             sorted_vision_embeddings = sorted_vision_embeddings.transpose(1, 2)
#             sorted_vision_embeddings = sorted_vision_embeddings.reshape(_b, 2 * _l, _h)
        
#         sorted_vision_embeddings = sorted_vision_embeddings.permute(0, 2, 1)
#         if self.kernel_size is not None:
#             sorted_vision_embeddings = torch.nn.functional.avg_pool1d(sorted_vision_embeddings, kernel_size=self.kernel_size, stride=self.stride, padding=0)
#         sorted_vision_embeddings = sorted_vision_embeddings.permute(0, 2, 1)
        
#         reduced_length = sorted_vision_embeddings.shape[1]
#         sorted_vision_embeddings = sorted_vision_embeddings. \
#             reshape(batch_size, self.num_head, reduced_length, head_dim). \
#             transpose(1, 2). \
#             reshape(batch_size, reduced_length, self.num_head * head_dim)
        
#         sorted_vision_embeddings = torch.concat([cls_embs, sorted_vision_embeddings], dim=1)
#         sorted_vision_embeddings = self.linear(sorted_vision_embeddings)
#         sorted_vision_embeddings = self.act(sorted_vision_embeddings)
        
#         return sorted_vision_embeddings
...
class M3AETransformerSS(pl.LightningModule):
    def __init__(self, config):
        ...
        if self.is_clip:
            self.vision_encoder = build_model(
                config["vit"], resolution_after=resolution_after
            )
        else:
            self.vision_encoder = getattr(swin, self.hparams.config["vit"])(
                pretrained=True, config=self.hparams.config
            )
        ...
        # ----------- Apply VDZ Here -----------
        self.multi_modal_vision_layers = nn.ModuleList(
            [BertCrossLayerWithVDZ(bert_config, True) for _, idx in enumerate(range(config["num_top_layer"]))]
        )
        # --------------------------------------
        ...
        self.multi_modal_language_pooler = prediction_heads.Pooler(
            config["hidden_size"]
        )
        ...
        if (
            ...
            or self.hparams.config["loss_names"]["irtr"] > 0
        ):
        ...
        if self.hparams.config["loss_names"]["cls"] > 0:
            ms = self.hparams.config["melinda_label_size"][
                self.hparams.config["label_column_name"]
            ]
    ...
    def infer(
        ...
        if mask_image:
            ...
            uni_modal_image_feats, mim_masks, mim_ids_restore = self.random_masking(
                uni_modal_image_feats, self.hparams.config["mim_prob"]
            )
            uni_modal_image_feats = self.vision_encoder.forward_trans(
        ...
        # == Begin: Use VDZ ==
        # if self.use_vdz:
        #     # print(f'[uni_modal_text_feats.shape] : {uni_modal_text_feats.shape}')
        #     # print(f'[uni_modal_image_feats.shape] : {uni_modal_image_feats.shape}')
        #     # exit(-1)
        #     uni_modal_image_feats = self.vdz(uni_modal_image_feats, uni_modal_text_feats)
        # == End  : Use VDZ ==
        ...
        ret["attentions"] = (
            {"text2image_attns": [], "image2text_attns": []}
        ...
        for layer_idx, (text_layer, image_layer) in enumerate(
            ...
            if mask_image and self.hparams.config["mim_layer"] == layer_idx:
                (
                    ...
                    ret[f"multi_modal_image_feats_{layer_idx}"],
                ) = (x, y)
        ...
        if self.is_clip:
            ...
            avg_image_feats = self.vision_pooler(
                multi_modal_image_feats.transpose(1, 2)
            ).view(multi_modal_image_feats.size(0), 1, -1)
            multi_modal_image_cls_feats = self.multi_modal_vision_pooler(
                avg_image_feats
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        total_loss = sum(
            [
                v * self.hparams.config["loss_names"][k.replace("_loss", "")]
    ...
    def test_step(self, batch, batch_idx):
        ...
        if False:
            if not hasattr(self, 'test_step_abc'):
                self.test_step_abc = []
            self.test_step_abc.append(output)
            torch.save(self.test_step_abc, 'temp/current_test_output.pth')
        if True:
            if not hasattr(self, 'test_step_index_batch'):
                self.test_step_index_batch = 0
            self.test_step_index_batch += 1
            torch.save(batch, f'temp/batch_{self.test_step_index_batch :03}.pth')
