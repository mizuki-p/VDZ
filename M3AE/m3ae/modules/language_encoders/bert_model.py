...
# Use this class to replace BertCrossLayer in BertLayer, and add the VDZ forward method
class BertCrossLayerWithVDZ(nn.Module):
    def __init__(self, config, enable_vdz=False):
        super().__init__()
        self.chunk_size_feed_forward = config.chunk_size_feed_forward
        self.seq_len_dim = 1
        self.attention = BertAttention(config)
        ...
        if self.enable_vdz:
            ...
            self.up_proj = nn.Linear(self.head_dim, int(self.head_dim * 4), bias=False)
    ...
    def forward(
        ...
        if self.enable_vdz:
            layer_output = self.vdz_forward(layer_output, encoder_hidden_states, attention_mask, encoder_attention_mask)
    ...
    def simple_linear_forward(self, hidden_states):
    ...
    def vdz_forward(
        ...
        _, q_seq_len, _ = encoder_hidden_states.shape
        q_states = encoder_hidden_states.view(batch_size, q_seq_len, self.num_heads, self.head_dim).transpose(1, 2)
        ...
        for i in range(batch_size):
            ...
            for j in range(self.num_heads):
                ...
                row, col = n // 13, n % 13
                temp = torch.zeros(1, 1, 24, 24, dtype=torch.bool).to(hidden_states.device)
                temp[:, :, row: row + 12, col: col + 12] = True
                
                temp = temp.view(-1)[torch.randperm(temp.nelement())].view(temp.size())
                _mask.append(temp)
        ...
        image_states = image_states[mask].reshape(batch_size, self.num_heads, 12, 12, self.head_dim)
        image_states = self.up_proj(image_states) # [batch_size, num_heads, pooled_width, pooled_height, head_dim * ratio * ratio]
        image_states = image_states.reshape(batch_size, self.num_heads, 12, 12, 2, 2, self.head_dim)
        image_states = image_states.transpose(3, 4).reshape(batch_size, self.num_heads, 24 * 24, self.head_dim)
        
        image_states = image_states.transpose(1, 2).reshape(batch_size, 24 * 24, hidden_size)
