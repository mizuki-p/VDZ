...
def prepro_vqa_ovqa():
    ...
    data_root = 'data/finetune_data/OVQA_publish'
    image_root = 'data/finetune_data/OVQA_publish/img'
    for split, file in zip(['train', 'val', 'test'], ['trainset.json', 'valset.json', 'testset.json']):
        with open(f'{data_root}/{file}', 'r') as f:
            samples = json.load(f)
            for sample in samples:
                img_path = os.path.join(image_root, sample["image_name"])
                qid = sample["qid"]
    ...
    make_arrow_vqa(data, 'vqa_ovqa', 'data/finetune_arrows/')
