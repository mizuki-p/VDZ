...
# with init_empty_weights():
    # model = create_base_model(BaseModelType[training_args.model], load_parameters=False)
# model = load_checkpoint_and_dispatch(model, saving_args.pretrained_model_path, device_map='auto', no_split_module_classes=['LlamaDecoderLayer', 'vision_model'])
# model = create_base_model(BaseModelType[training_args.model], None, load_parameters=True)

model = PaliGemmaForConditionalGeneration.from_pretrained('models/paligemma-3b-ft-vqav2-448')
model = PeftModel.from_pretrained(
    model, 
    saving_args.pretrained_model_path,
    is_trainable=True
)
print(model)
model.train()
