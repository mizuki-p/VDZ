...
class GemmaAttention(nn.Module):
    ...
    def forward(
        ...
        if attention_mask is not None and attention_mask.sum() != 0:  # no matter the length, we just slice it
...
class VDZ(nn.Module):
    def __init__(self, config: GemmaConfig, layer_idx):
        ...
        self.vision_tokens = 32 * 32
        self.scale_ratio = 2
        self.image_edge = 32
        self.pool_kernel_size = 32 // 2
        self.pooled_edge = 1 + 32 - self.pool_kernel_size
...
def check_layer_idx_to_add_vdz_module(li):
    return li % 2 == 0
# ============== VDZ module ==============
...
class GemmaDecoderLayer(nn.Module):
    def __init__(self, config: GemmaConfig, layer_idx: int):
        ...
        if UseVDZ and check_layer_idx_to_add_vdz_module(layer_idx):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if UseVDZ and check_layer_idx_to_add_vdz_module(self.layer_idx):
            image_states = hidden_states[:, :1024, :]
            text_states = hidden_states[:, 1024:, :]
            image_states = self.vdz(image_states, text_states)
            hidden_states = torch.cat([image_states, text_states], dim=1)
