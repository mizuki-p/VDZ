...
from transformers.activations import ACT2FN, QuickGELUActivation
...
from transformers.models.deprecated._archive_maps import BRIDGETOWER_PRETRAINED_MODEL_ARCHIVE_LIST  # noqa: F401, E402
...
UseVDZ = True
class VDZ(nn.Module):
    def __init__(self, config: BridgeTowerConfig, layer_idx):
        super(VDZ, self).__init__()
        
        self.layer_idx = layer_idx
        
        self.vision_tokens = 18 * 18
        self.scale_ratio = 2
        self.image_edge = 18
        self.pool_kernel_size = 18 // 2
        self.pooled_edge = 1 + 18 - self.pool_kernel_size
        
        self.hidden_size = config.hidden_size
        self.num_heads = config.num_attention_heads
        self.head_dim = self.hidden_size // self.num_heads
        self.attention_dropout = .1
        ...
        self.input_proj = nn.Linear(self.hidden_size, self.hidden_size, bias=True)
    ...
    def forward(
        ...
        image_states = image_states[:, :, 1:, :]
        cls_token = image_hidden_states[:, 0:1, :]
        
        image_attn_softmax = nn.functional.softmax(image_attn, dim=-1)
        image_attn_softmax = image_attn_softmax.reshape(batch_size, self.num_heads, self.image_edge, self.image_edge)
        ...
        image_hidden_states = torch.concat([
            cls_token,
            image_hidden_states
        ], dim=1)
...
def check_layer_idx_to_add_ir_module(li):
    return UseVDZ and li is not None and li % 2 == 1
# ======== VDZ module ========
...
class BridgeTowerBertCrossLayer(nn.Module):
    def __init__(self, config, layer_idx=None):
        ...
        if check_layer_idx_to_add_ir_module(layer_idx):
            self.vdz = VDZ(config, layer_idx)
    ...
    def forward(
        ...
        if hasattr(self, 'vdz'):
            layer_output = self.vdz(layer_output, encoder_hidden_states)
...
class BridgeTowerModel(BridgeTowerPreTrainedModel):
    def __init__(self, config):
        ...
        self.cross_modal_image_layers = nn.ModuleList(
            [BridgeTowerBertCrossLayer(text_config, i) for i in range(config.num_hidden_layers)]
        )
...
class BridgeTowerForVisualQuestionAnswering(BridgeTowerPreTrainedModel):
    def __init__(self, config: BridgeTowerConfig):
        super().__init__(config)

        self.bridgetower = BridgeTowerModel(config)

        self.classify_head = ClassificationHead(config.hidden_size * 2, 476)
    ...
    def forward(
        ...
        pooler_output = outputs.pooler_output if return_dict else outputs[2]
        logits = self.classify_head(pooler_output)
